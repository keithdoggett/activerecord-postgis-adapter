...
class SpatialQueriesTest < ActiveSupport::TestCase
  ...
  def test_query_multi_point
    create_model
    obj = SpatialModel.create!(latlons: factory.multi_point([factory.point(1, 2)]))
    id = obj.id
    obj2 = SpatialModel.find_by(latlons: factory.multi_point([factory.point(1, 2)]))
    refute_nil(obj2)
  ...
  def create_model
    SpatialModel.connection.create_table(:spatial_models, force: true) do |t|
      ...
      t.column "latlons", :multi_point, srid: 3785
